...
# =========================================================================
# Step 4.7 -- real, enforced Grafana authentication config (closing the
# Stage 3 gap: the instance tested there was a drifted, pre-existing dev
# artifact dated 3 weeks before this packaging effort even started --
# confirmed via its own mtime -- not something install.sh ever created or
# configured; this project's own original design was "real auth enforced
# regardless, no anonymous-access path, ever," and install.sh/run.sh never
# actually implemented that -- they only detected+warned about whatever
# instance already happened to be running). Generates a real config
# fragment (never launches Grafana itself, matching Step 4.5's/vm-
# standalone's own established scope boundary -- see grafana-standalone/
# README.md for the real, confirmed launch command that uses this) with
# anonymous access explicitly disabled and a real, random admin password
# -- never a hardcoded default. Idempotent: re-running install.sh reuses
# an already-generated real password instead of silently invalidating an
# operator's already-established login.
# =========================================================================

info "Generating real, enforced Grafana authentication config..."
...
if [ -f "$GRAFANA_ADMIN_CREDENTIALS_FILE" ]; then
  ...
  info "Reusing the already-generated real admin password from a prior install.sh run ($GRAFANA_ADMIN_CREDENTIALS_FILE) instead of invalidating it."
fi
if [ -z "${_admin_pw:-}" ]; then
  _admin_pw="$(python3 -c "import secrets; print(secrets.token_urlsafe(18))")"
  [ -z "$_admin_pw" ] && fail "could not generate a real random admin password (python3 secrets module failed)."
fi
cat > "$GRAFANA_ADMIN_CREDENTIALS_FILE" <<EOF
# Generated by install.sh at $(date -u +%FT%TZ) -- real, randomly-generated
# Grafana admin credentials. Never a hardcoded default. Restricted to this
# file's own owner (chmod 600 below) -- retrieve with:
#   cat $GRAFANA_ADMIN_CREDENTIALS_FILE
admin_user: admin
admin_password: $_admin_pw
EOF
chmod 600 "$GRAFANA_ADMIN_CREDENTIALS_FILE"
cat > "$GRAFANA_AUTH_CONFIG_FILE" <<EOF
# Generated by install.sh at $(date -u +%FT%TZ) -- real, enforced auth.
# Merge/append this into a fresh Grafana instance's own config (e.g. via
# --config, or appended to custom.ini before first start) -- see
# grafana-standalone/README.md for the exact real, confirmed launch
# command. admin_user/admin_password here MUST match
# $GRAFANA_ADMIN_CREDENTIALS_FILE and only take effect on a genuinely
# fresh instance's FIRST start (Grafana ignores these once its own sqlite
# DB already exists -- use \`grafana-cli admin reset-admin-password\` on an
# already-initialized instance instead).
[auth.anonymous]
enabled = false
[security]
admin_user = admin
...
if [ "${GRAFANA_ANON_ENABLED:-unknown}" = "true" ]; then
  warn "The CURRENTLY reachable instance at $GRAFANA_URL still has anonymous access enabled (see the WARNING above) -- this generated config is not yet applied there. See grafana-standalone/README.md to launch a properly-configured fresh instance instead."
fi
...
GRAFANA_ADMIN_CREDENTIALS_FILE="$GRAFANA_ADMIN_CREDENTIALS_FILE"
GRAFANA_AUTH_CONFIG_FILE="$GRAFANA_AUTH_CONFIG_FILE"
EOF
