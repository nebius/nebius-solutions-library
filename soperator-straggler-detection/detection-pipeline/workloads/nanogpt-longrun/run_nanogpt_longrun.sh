...
echo "### nanogpt-longrun steps=$STEPS outdir=$OUTDIR port=$PORT"
echo "--- queue check ---"
...
srun --nodes=2 --ntasks=2 --ntasks-per-node=1 --gpus-per-node=8 -w worker-0,worker-1 \
  ...
  bash -c 'if [ "$(hostname)" = "worker-0" ]; then DD='"$DUMPDIR_W0"'; else DD='"$DUMPDIR_W1"'; fi; bash /root/P20c_prereqs/train_node_nanogpt_longrun.sh '"$STEPS $OUTDIR $PORT"' $DD' \
