...
# P20k -- STRAGGLER_SLEEP_MS / STRAGGLER_TARGET_RANKS are read directly by
# the patched train.py (own copy, not the shared P3_real_workload one) --
...
torchrun --nnodes="$NUM_NODES" --nproc_per_node="$GPUS_PER_NODE" --node_rank=$RANK \
  --rdzv_id=p20k_straggler --rdzv_backend=c10d --rdzv_endpoint=$RDZV_HOST:$PORT \
