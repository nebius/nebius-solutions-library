...
srun --nodes=2 --ntasks=2 --ntasks-per-node=1 --gpus-per-node=8 -w worker-0,worker-1 \
  ...
  --export=ALL,STRAGGLER_MODE=file_trigger,STRAGGLER_TRIGGER_FILE=/root/_v1beta_dryrun/trigger.json \
  bash -c 'if [ "$(hostname)" = "worker-0" ]; then DD='"$DUMPDIR_W0"'; else DD='"$DUMPDIR_W1"'; fi; bash /root/_v1beta_dryrun/train_node_shape1.sh '"$STEPS $OUTDIR $PORT"' $DD' \
