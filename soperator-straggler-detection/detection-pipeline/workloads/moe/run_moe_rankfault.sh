...
echo "### moe-rankfault steps=$STEPS outdir=$OUTDIR port=$PORT (per-rank NCCL_MAX_NCHANNELS=1, target rank ${FAULT_TARGET_RANK:-4})"
echo "--- queue check ---"
...
srun --nodes=2 --ntasks=2 --ntasks-per-node=1 --gpus-per-node=8 -w worker-0,worker-1 \
  ...
  --export=ALL \
  bash -c 'if [ "$(hostname)" = "worker-0" ]; then DD='"$DUMPDIR_W0"'; else DD='"$DUMPDIR_W1"'; fi; bash /root/P23_moe/train_node_moe_rankfault.sh '"$STEPS $OUTDIR $PORT"' $DD' \
