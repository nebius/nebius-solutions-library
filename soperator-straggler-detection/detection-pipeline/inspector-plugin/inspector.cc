...
#include <errno.h>
#include <cstring>
// P21.7 -- needed for cudaGetDevice()/cudaError_t (see g_cudaDeviceOrdinal's
// own comment). Not transitively available via this plugin's own local
// nccl/ header bundle (nccl/common.h, nccl/profiler.h) -- confirmed
// directly those are self-contained and never include NCCL's real public
// nccl.h (which is where this would otherwise come from transitively).
...
// P21.7 -- the real physical GPU device ordinal (0-7 on an 8-GPU node)
// this process is bound to. Captured EXACTLY ONCE, synchronously inside
// inspectorGlobalInit() -- which NCCL calls from the SAME thread that
// calls ncclCommInitRank (the training script's own thread, which has
// already called cudaSetDevice()/torch.cuda.set_device() before NCCL init
// runs at all). This is deliberately NOT captured lazily at JSON-dump
// time: dumping happens on a separate, internally-spawned pthread
// (inspectorDumpThread, see below) that never itself set a device
// binding, so querying the device from THAT thread would return CUDA's
// per-thread default (device 0) regardless of which GPU this process
// actually uses -- confirmed by reading NCCL's own profiler API headers
// (nccl/profiler_v5.h) and this file's own dump-thread structure before
// writing a single line of the fix.
//
// Calls cudaGetDevice() directly (the CUDA RUNTIME API) -- matching
// NCCL's OWN established precedent exactly: its core source
// (src/misc/cudawrap.cc, ncclIsCuMemSupported()) calls cudaGetDevice()/
// cudaDriverGetVersion() the same direct way, with no dlopen/dlsym
// wrapper. Confirmed via `ldd` that the resulting libnccl.so has ZERO
// CUDA-related linked (NEEDED) dependencies at all, despite calling
// these functions directly -- shared libraries permit unresolved
// symbols by default (no -Wl,--no-undefined here or in NCCL's own
// build), deferred to whatever the FINAL loading process (the training
// script, which already has libcudart loaded for its own CUDA use)
// provides at load time. This is exactly why this plugin (built once on
// this login node, one CUDA toolkit version) safely dlopen()'s inside
// training containers that may bundle a DIFFERENT toolkit version --
// same reasoning already applies to libnccl.so itself, not a new
// concern introduced here. No new link-time library dependency added.
//
// -1 is the explicit "not captured" sentinel (the call itself failed) --
// never silently defaulted to a plausible-looking wrong value like 0.
static int g_cudaDeviceOrdinal = -1;
...
static inspectorResult_t inspectorCommInfoMetaHeader(jsonFileOutput* jfo) {
  JSON_CHK(jsonStartObject(jfo));
  {
    ...
    // P21.7 -- real physical CUDA device ordinal (0-7 on an 8-GPU node),
    // captured once on the correct thread in inspectorGlobalInit (see
    // g_cudaDeviceOrdinal's own comment) -- just read here, no new CUDA
    // call from this (dump) thread, which does not have the right
    // context bound. -1 means "not captured" / cudaGetDevice failed --
    // an honest sentinel, never a guessed value.
    JSON_CHK(jsonKey(jfo, "gpu_slot_index")); JSON_CHK(jsonInt(jfo, g_cudaDeviceOrdinal));
  }
  JSON_CHK(jsonFinishObject(jfo));
  return inspectorSuccess;
}
...
inspectorResult_t inspectorGlobalInit(int rank) {
  ...
  // P21.7 -- capture the real physical GPU ordinal here, on this thread,
  // once. See g_cudaDeviceOrdinal's own comment for why here specifically
  // (this thread has the correct CUDA context bound; the internal dump
  // thread spawned later does not) and why via a direct cudaGetDevice()
  // call rather than a dlsym wrapper (matches NCCL's own precedent
  // exactly, see that comment for the full reasoning). A failure here is
  // a real, honestly-reported "unknown" (-1), not silently treated as
  // device 0.
  {
    cudaError_t cudaErr = cudaGetDevice(&g_cudaDeviceOrdinal);
    if (cudaErr != cudaSuccess) {
      INFO(NCCL_INSPECTOR, "NCCL Inspector: cudaGetDevice failed ('%s') -- "
           "gpu_slot_index will report -1 (unknown) for this process",
           cudaGetErrorString(cudaErr));
      g_cudaDeviceOrdinal = -1;
    }
  }
