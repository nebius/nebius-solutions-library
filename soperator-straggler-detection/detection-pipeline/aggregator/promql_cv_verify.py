...
def export_all_ranks(bucket, start, end, hostname_filter=None):
...
if __name__ == "__main__":
    import sys
    bucket, start, end, label = sys.argv[1], int(sys.argv[2]), int(sys.argv[3]), sys.argv[4]
    data = export_all_ranks(bucket, start, end)
    node_a = {r for r in data if int(r) < 8}
    fired, idx, _ = cv_persistence_check(data, EXCLUDE_ALWAYS | EXCLUDE_CV_EXTRA, node_a)
    print(f"[{label}] main CV check (node A): fired={fired} at window {idx}")
